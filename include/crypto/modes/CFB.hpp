...
namespace crypto::modes {
    class CFB : public ICipherMode {
        Bytes iv;
    public:
        // CFB - потоковый режим, паддинг не обязателен, но для сохранения архитектуры
        // обычно делают Full Block CFB, который требует кратности блока или паддинга.
        // Реализуем вариант с паддингом.
        ...
        Bytes encrypt(ConstBytesSpan input) override {
            ...
            if (padding) padding->addPadding(data, bs); // Опционально

            Bytes result(data.size());
            Bytes feedback = iv;

            for (size_t i = 0; i < data.size(); i += bs) {
                // Шифруем предыдущий шифротекст (или IV)
                Bytes output(bs);
                ...
                for (size_t j = 0; j < len; ++j) {
                    ...
                    // Для CFB feedback - это результат (шифротекст)
        ...
        Bytes decrypt(ConstBytesSpan input) override {
            ...
            for (size_t i = 0; i < input.size(); i += bs) {
                Bytes output(bs);
                cipher->encryptBlock(feedback, output); // ENCRYPT для дешифрации CFB!

                size_t len = std::min(bs, input.size() - i);
                for (size_t j = 0; j < len; ++j) {
                    Byte c = input[i + j];
                    result[i + j] = c ^ output[j];
                    // Для CFB feedback - это входящий шифротекст
